...
public class FoliaGlobalRegionScheduler extends FoliaScheduler {
    @Override
    ...
    public Task run(@NotNull Consumer<Task> task,
        ...
        return executeRun((scheduler, wrappedTask) -> scheduler
                .run(Schedulers.OWNING_PLUGIN, unwrapTaskConsumer(task, wrappedTask)));
    }
    ...
    public Task runDelayed(@NotNull Consumer<Task> task,
        ...
        return executeRun((scheduler, wrappedTask) -> scheduler
                .runDelayed(Schedulers.OWNING_PLUGIN, unwrapTaskConsumer(task, wrappedTask), delayTicks));
    }
    ...
    public Task runAtFixedRate(@NotNull Consumer<Task> task,
        ...
        return executeRun((scheduler, wrappedTask) -> scheduler
                .runAtFixedRate(Schedulers.OWNING_PLUGIN, unwrapTaskConsumer(task, wrappedTask), initialDelayTicks, periodTicks));
    }

    private Task executeRun(BiFunction<GlobalRegionScheduler, FoliaTask, ScheduledTask> method) {
        FoliaTask wrappedTask = new FoliaTask();
        ScheduledTask foliaTask = method.apply(Bukkit.getGlobalRegionScheduler(), wrappedTask);

...
public class BukkitScheduler implements Scheduler {
    ...
    public boolean execute(@NotNull Runnable run, @Nullable Runnable retired,
                           long delay) {
        try {
            return !Bukkit.getScheduler().runTaskLater(Schedulers.OWNING_PLUGIN, run, delay).isCancelled();
    ...
    public @Nullable Task runDelayed(@NotNull Consumer<Task> task,
        ...
        try {
            ...
            Bukkit.getScheduler().runTaskLater(Schedulers.OWNING_PLUGIN, bukkitTask -> {
    ...
    public @Nullable Task runAtFixedRate(@NotNull Consumer<Task> task,
        ...
        try {
            ...
            Bukkit.getScheduler().runTaskTimer(Schedulers.OWNING_PLUGIN, bukkitTask -> {
                if (!bukkitTask.isCancelled()) {
                    t.provideHandle(bukkitTask);
                    task.accept(t);
                } else if (retired != null) {

...
final class FoliaScheduler implements Scheduler {
    ...
    FoliaScheduler(WorldGuardPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void runGlobalAtFixedRate(Runnable task, long delay, long period) {
        plugin.getServer().getGlobalRegionScheduler().runAtFixedRate(plugin, $ -> task.run(), delay, period);
    }

    @Override
    public void runAtRegion(Location location, Runnable task) {
        plugin.getServer().getRegionScheduler().run(plugin, location, $ -> task.run());
    }

    @Override
    public void runAsyncAtFixedRate(Runnable task, long delay, long period) {
        plugin.getServer().getAsyncScheduler().runAtFixedRate(plugin, $ -> task.run(), Tick.of(delay).toMillis(), Tick.of(period).toMillis(), TimeUnit.MILLISECONDS);
    }
